...
public class TaskDistributeInHandler extends SimpleChannelInboundHandler<RequestMsg> {
    ...
    public void init() {
        ...
        handlerMap.put(MsgType.TMT_UpdateQuestionReq,
                (msg, ctx) -> handle(
                        msg.getToken(),
                        msg.getMsg().toByteArray(),
                        UpdateQuestionReq.parser(),
                        requestTransfer::updateQuestion,
                        ctx
                ));
        handlerMap.put(MsgType.TMT_ListQuestionReq,
                (msg, ctx) -> handle(
                        msg.getToken(),
                        msg.getMsg().toByteArray(),
                        ListQuestionReq.parser(),
                        requestTransfer::listQuestion,
                        ctx
                ));

        /// study
        handlerMap.put(MsgType.TMT_CreateStudyReq,
                (msg, ctx) -> handle(
                        msg.getToken(),
                        msg.getMsg().toByteArray(),
                        CreateStudyReq.parser(),
